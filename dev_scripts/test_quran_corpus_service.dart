...
Future<void> testQuranCorpusService() async {
  try {
    ...
    final verses = data
        .where((v) =>
            v['surah'] == surah && v['ayah'] >= start && v['ayah'] <= end)
        .toList();
    ...
    for (final v in verses) {
      ...
      print('Verset $ayah:');
      print(
          '  AR: ${textAr.substring(0, textAr.length > 50 ? 50 : textAr.length)}...');
      print(
          '  FR: ${textFr.substring(0, textFr.length > 50 ? 50 : textFr.length)}...');
    }
    ...
    final hasBasmalah =
        result.contains('بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ');
    ...
    if (hasBasmalah && hasFirstVerse) {
      ...
      print(
          '🔍 Il faut chercher ailleurs : interface utilisateur, état, callback...');
    } else {

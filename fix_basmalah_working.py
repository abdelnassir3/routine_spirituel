...
# -*- coding: utf-8 -*-
"""
Script fonctionnel pour corriger les versets 1 en ajoutant un retour à la ligne après la Basmalah
"""
import json
...
def fix_basmalah_verses(input_file, output_file):
    ...
    for verse in data:
        if verse.get('ayah') == 1:  # Seulement les premiers versets
            ...
            # Vérifier si on a au moins 5 mots et que le premier est "بِسْمِ"
            if len(words) >= 5 and words[0] == 'بِسْمِ':
                # Les 4 premiers mots forment la Basmalah
                basmalah = ' '.join(words[:4])

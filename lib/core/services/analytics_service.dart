...
class AnalyticsService {
    ...
    if (previousMetrics.totalRepetitions > 0) {
      metrics.progressionPercent =
          ((metrics.totalRepetitions - previousMetrics.totalRepetitions) /
                  previousMetrics.totalRepetitions *
                  100)
              .round();
    }
  ...
  Future<void> _saveMilestone(
      String type, int value, String? description) async {
...
class DailyMetrics {
  ...
  double get completionRate =>
      sessionsStarted > 0 ? sessionsCompleted / sessionsStarted : 0;

  double get averageSessionDuration =>
      sessionsCompleted > 0 ? totalDuration / sessionsCompleted : 0;

...
class ApiKeyInitializer {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      if (existingKey == null ||
          existingKey.isEmpty ||
          existingKey !=
              '8e88bed4c0c2e2f35e4b7e96e967c2a7fd1e7e21e582b14c1f2b81a983b8b9e1') {
        // Configuration initiale avec votre API key
        await _storage.write(
            key: 'tts_coqui_endpoint', value: 'http://168.231.112.71:8001');
        ...
        await _storage.write(
            key: 'tts_preferred_provider', value: 'coqui'); // Coqui par défaut
  ...
  static Future<Map<String, String>> getConfig() async {
    try {
      return {
        'endpoint':
            await _storage.read(key: 'tts_coqui_endpoint') ?? 'Non configuré',
        'apiKey':
            (await _storage.read(key: 'tts_coqui_api_key') ?? '').isNotEmpty
                ? 'Configurée'
                : 'Non configurée',
        'provider':
            await _storage.read(key: 'tts_preferred_provider') ?? 'flutter_tts',
      };
    } catch (e) {
      return {'status': 'Erreur: $e'};
    }
  }
}

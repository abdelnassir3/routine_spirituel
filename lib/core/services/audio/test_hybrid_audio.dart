...
class HybridAudioTest {
  ...
  static Future<void> _testQuranicContent(SmartTtsEnhancedService tts) async {
    ...
    const quranicText = '{{V:1:1}} بسم الله الرحمن الرحيم {{V:1:2}} الحمد لله رب العالمين';
  ...
  static Future<void> _testFrenchContent(SmartTtsEnhancedService tts) async {
    ...
    const frenchText = 'Bonjour, ceci est un test de synthèse vocale en français avec Edge-TTS.';
    
    final analysis = tts.analyzeContent(frenchText);
    debugPrint('📊 Analyse: ${tts.previewContentType(frenchText)}');
    debugPrint('🎯 Type détecté: ${analysis.contentType}');
    debugPrint('🗣️ Ratio français: ${(analysis.languageRatio.french * 100).round()}%');
  ...
  static Future<void> _testIslamicDua(SmartTtsEnhancedService tts) async {
    ...
    const duaText = 'بسم الله الرحمن الرحيم، الحمد لله رب العالمين، أستغفر الله العظيم';
  ...
  static Future<void> _testMixedContent(SmartTtsEnhancedService tts) async {
    ...
    const mixedText = 'Voici une invocation: بسم الله الرحمن الرحيم - Au nom d\'Allah, le Tout Miséricordieux';
    
    final analysis = tts.analyzeContent(mixedText);
    debugPrint('📊 Analyse: ${tts.previewContentType(mixedText)}');
    debugPrint('🎯 Type détecté: ${analysis.contentType}');
    debugPrint('🌐 Ratio AR/FR: ${(analysis.languageRatio.arabic * 100).round()}% / ${(analysis.languageRatio.french * 100).round()}%');

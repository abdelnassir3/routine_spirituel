...
class VpsConnectionTest {
  ...
  static Future<void> _testBasicConnection(VpsTestResult result) async {
    ...
    try {
      ...
      result.basicConnection = response.statusCode != null && response.statusCode! < 500;
      
      debugPrint('✅ Connexion de base: ${response.statusCode} (${result.latency}ms)');
  ...
  static Future<void> _testHealthEndpoint(VpsTestResult result) async {
    try {
      ...
      debugPrint('⚠️ Health endpoint non disponible (normal si pas implémenté)');
    }
  }
  ...
  static Future<void> _testApiKey(VpsTestResult result) async {
    try {
      ...
      debugPrint('⚠️ Impossible de valider l\'API Key (endpoint test non disponible)');
    }
  }

...
class AudioHybridTestService {
  ...
  static Future<void> _testQuranicContent(AudioServiceHybridWrapper service) async {
    ...
    for (final text in testTexts) {
      try {
        TtsLogger.info('🎵 Test verset', {'text': text.substring(0, 30) + '...'});
  ...
  static Future<void> _testIslamicDua(AudioServiceHybridWrapper service) async {
    ...
    for (final text in testTexts) {
      try {
        TtsLogger.info('🎵 Test invocation', {'text': text.substring(0, 30) + '...'});
        
        final analysis = await HybridAudioService.analyzeContentDetails(text);
        TtsLogger.info('📊 Analyse invocation', {
          'type': analysis.contentType.toString(),
          'arabicRatio': (analysis.languageRatio.arabic * 100).toStringAsFixed(1) + '%',
        });
    ...
    for (final text in testTexts) {
      try {
        ...
        TtsLogger.info('📊 Analyse texte français', {
          'type': analysis.contentType.toString(),
          'frenchRatio': (analysis.languageRatio.french * 100).toStringAsFixed(1) + '%',
        });
        ...
        TtsLogger.error('❌ Erreur test texte français', {'error': e.toString()});
      }
    }
  }
  
  /// Test 5: Contenu mixte
  static Future<void> _testMixedContent(AudioServiceHybridWrapper service) async {
  ...
  static Future<void> testQuranicSpeedAdjustment() async {
    ...
    for (final speed in testSpeeds) {
      ...
      TtsLogger.info('🎵 Ajustement vitesse', {
        ...
        'reduction': '${((1 - adjustedSpeed / speed) * 100).toStringAsFixed(1)}%',
      });
    }
  }
  ...
  static Future<void> performanceTest(AudioServiceHybridWrapper service) async {
    ...
    TtsLogger.info('🏁 Performance totale', {
      ...
      'averageTime': '${(totalTime.inMilliseconds / testTexts.length).toStringAsFixed(1)}ms',
    });
  }
}

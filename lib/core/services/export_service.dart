...
class ExportService {
  ...
  Future<String> _generateCSV(ExportData data, bool includeDetails) async {
    ...
    // En-tête
    rows.add([
      'Statistiques spirituelles - Export du ${_formatDate(DateTime.now())}'
    ]);
    rows.add([]);
    ...
    if (data.allTimeStats != null) {
      ...
      rows.add([
        'Temps de pratique (heures)',
        (data.allTimeStats!.totalDuration / 3600).toStringAsFixed(1)
      ]);
    ...
    if (data.streakData != null) {
      ...
      rows.add([
        'Dernière activité',
        _formatDate(data.streakData!.lastActivityDate)
      ]);
      rows.add([]);
    }
  ...
  Future<String> _generateJSON(ExportData data, bool prettyPrint) async {
    ...
    if (data.dailyMetrics.isNotEmpty) {
      jsonData['details']['daily'] =
          data.dailyMetrics.map((d) => d.toJson()).toList();
    }

    // Milestones
    if (data.milestones.isNotEmpty) {
      jsonData['details']['milestones'] =
          ...
          return pw.Column(
            ...
            children: [
              ...
              if (data.allTimeStats != null) ...[
                ...
                _buildPDFStat(
                    'Jours de pratique', '${data.allTimeStats!.totalDays}'),
              ],
              ...
              if (data.streakData != null) ...[
                ...
                _buildPDFStat(
                    'Record', '${data.streakData!.longestStreak} jours'),
              ],
            ],
          );
        },
      ...
      switch (type) {
        ...
        case ExportDataType.prayerDistribution:
          data.prayerDistribution =
              await _analyticsService.getPrayerDistribution();
          break;
      }
    }
  ...
  Future<Map<String, List<ChartData>>> _collectChartData(

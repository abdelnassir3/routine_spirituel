...
class SecureLoggingService {
  ...
  static final List<RegExp> _piiPatterns = [
    ...
    RegExp(r'\b(?:\+?[1-9]\d{0,2}[\s.-]?)?\(?\d{1,4}\)?[\s.-]?\d{1,4}[\s.-]?\d{1,4}\b'),
    // Carte de crédit
    ...
    // Clés API communes
    RegExp(r'(api[_-]?key|apikey|api_secret|access[_-]?token|auth[_-]?token|authorization)\s*[:=]\s*["\']?[\w\-]+["\']?', caseSensitive: false),
    // Mots de passe
    RegExp(r'(password|passwd|pwd|pass)\s*[:=]\s*["\']?[^"\']+["\']?', caseSensitive: false),
    // Adresse IP
  ...
  String _sanitizeText(String text) {
    ...
    for (final pattern in _piiPatterns) {
      sanitized = sanitized.replaceAllMapped(pattern, (match) {
        ...
        } else if (RegExp(r'^\+?\d').hasMatch(matchText) && matchText.length > 6) {
  ...
  void _printToConsole(LogEntry entry) {
    ...
    print('$color$emoji ${entry.level.name.toUpperCase()} [${entry.timestamp.toIso8601String()}]');
    print('  ${entry.message}');

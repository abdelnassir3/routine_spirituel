...
class FarasaTest {
  ...
  static Future<void> _testConnection() async {
    ...
    try {
      final isWorking =
          await FarasaDiacritizationService.testFarasaConnection();
      debugPrint(
          isWorking ? '✅ Connexion API réussie' : '❌ Connexion API échouée');
    ...
    for (final testCase in testCases) {
      try {
        ...
        if (hasHarakat) {
          ...
          debugPrint(
              '      ⚠️ Aucun harakat détecté (peut être normal si API indisponible)');
        }
      } catch (e) {
        debugPrint('      ❌ Erreur: $e');
      }
  ...
  static Future<void> _testSpecializedTexts() async {
    ...
    for (final testCase in testCases) {
      try {
        ...
        debugPrint(
            '      Original: ${input.substring(0, input.length.clamp(0, 50))}...');
        debugPrint(
            '      Diacritisé: ${result.substring(0, result.length.clamp(0, 50))}...');
  ...
  static Future<void> _testPerformance() async {
    ...
    try {
      ...
      debugPrint(
          '   📊 Temps: ${duration}ms pour ${testText.length} caractères');
      debugPrint(
          '   📈 Vitesse: ${(testText.length / duration * 1000).round()} chars/s');

      // Test du cache
      debugPrint('   💾 Test cache...');
      final cacheStopwatch = Stopwatch()..start();
      final cachedResult =
          await FarasaDiacritizationService.diacritizeText(testText);
      cacheStopwatch.stop();

      final cacheDuration = cacheStopwatch.elapsedMilliseconds;
      debugPrint(
          '   ⚡ Cache: ${cacheDuration}ms (gain: ${duration - cacheDuration}ms)');

...
class TtsLogger {
  static TtsLogLevel _minLevel = kDebugMode ? TtsLogLevel.debug : TtsLogLevel.info;
  ...
  static void error(String message, [Map<String, dynamic>? data, Object? error, StackTrace? stackTrace]) {
    final errorData = {
      ...
      if (stackTrace != null && kDebugMode) 'stackTrace': stackTrace.toString().split('\n').take(5).join('\n'),
    };
  ...
  static void _log(TtsLogLevel level, String message, Map<String, dynamic>? data) {
  ...
  static Map<String, dynamic> _sanitizeData(Map<String, dynamic> data) {
    return data.map((key, value) {
      ...
      if (key.toLowerCase().contains('text') || key.toLowerCase().contains('content')) {
        if (value is String && value.length > 100) {
          return MapEntry(key, '${value.substring(0, 50)}...[${value.length} chars]');
        }
      }
  ...
  static Map<String, dynamic> _sanitizeError(Object error) {
    ...
    return {
      ...
      'message': sanitized.length > 500 ? '${sanitized.substring(0, 500)}...' : sanitized,
    };
  }
}

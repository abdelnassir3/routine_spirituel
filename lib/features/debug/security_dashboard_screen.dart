...
class _SecurityDashboardScreenState
  ...
  Future<SecurityCheck> _checkConfiguration() async {
    ...
    if (AppConfig.isProduction) {
      issues.add(
          'Running in production mode - security dashboard should be disabled');
    } else {
  ...
  Future<SecurityCheck> _checkStorageSecurity() async {
    ...
    final hasToken =
        await storage.containsKey(key: SecureStorageService.keyAuthToken);
    if (hasToken) {
    ...
    final hasPin =
        await storage.read(key: SecureStorageService.keyPinCode) != null;
    if (hasPin) {
  ...
  Future<SecurityCheck> _checkLoggingSecurity() async {
    ...
    for (final log in recentLogs) {
      if (log.message.contains('@') &&
          !log.message.contains('[EMAIL_REDACTED]')) {
        foundPII = true;
        break;
      }
    }
  ...
  Future<SecurityCheck> _checkPermissions() async {
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      ...
      if (await libDir.exists()) {
        ...
        if (!foundCamera)
          passed.add('No camera access detected');
        else
          issues.add('Camera access detected - verify if necessary');

        if (!foundLocation)
          passed.add('No location access detected');
        else
          issues.add('Location access detected - verify if necessary');

        if (!foundContacts)
          passed.add('No contacts access detected');
        else
          issues.add('Contacts access detected - verify if necessary');
      }
    }
                  ...
                  ..._checks.map((check) => Card(
                        ...
                        child: ExpansionTile(
                          ...
                          trailing: Container(
                            ...
                            decoration: BoxDecoration(
                              color:
                                  _getScoreColor(check.score).withOpacity(0.2),

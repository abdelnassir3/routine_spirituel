...
Future<void> testSimplifiedBuildTextFromRefs() async {
  try {
    ...
    } else if (result.contains('بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ') && result.contains('قُلْ هُوَ ٱللَّهُ أَحَدٌ')) {
      print('⚠️ Basmalah et verset présents mais peut-être pas parfaitement séparés');
    } else {
  ...
  try {
    ...
    if (verse.contains('\n')) {
      ...
      if (firstPart == bismillah) {
        final restParts = parts.skip(1).where((p) => p.trim().isNotEmpty).toList();

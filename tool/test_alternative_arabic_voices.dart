...
void main() async {
  ...
  if (workingVoices.isNotEmpty) {
    ...
    print('🔧 Recommandation: ${workingVoices.first['voice']} comme voix par défaut');
  } else {
...
Future<bool> testArabicVoice(String endpoint, String apiKey, Map<String, String> voiceInfo) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      try {
        ...
        if (responseBody.length > 500) { // Audio probable
      ...
      final errorBody = await response.transform(utf8.decoder).join();
      print('   ❌ Erreur ${response.statusCode}: ${errorBody.substring(0, 50)}...');
      return false;
    }

  ...
  try {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('  📄 Réponse health: ${responseBody.substring(0, responseBody.length > 100 ? 100 : responseBody.length)}...');
      }
    } catch (e) {
    ...
    if (response.statusCode == 200) {
      ...
      if (totalBytes > 0) {
        ...
        if (firstChunk.length >= 4) {
          ...
          print('  🎵 Header MP3: ${isValidMp3 ? '✅ Valide' : '❌ Invalide'} (${header.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')})');
        }
      }
    } else {

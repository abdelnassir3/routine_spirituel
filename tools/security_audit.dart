...
Future<AuditResult> checkPermissions() async {
  ...
  if (await androidManifest.exists()) {
    ...
    for (final perm in expectedPerms) {
      if (!content.contains('android.permission.$perm')) {
        recommendations.add('Android: Consider adding permission $perm if needed');
      }
    }
  }
...
Future<AuditResult> analyzeCode() async {
  ...
  if (result.exitCode != 0) {
    ...
    if (infoCount > 10) {
      recommendations.add('$infoCount info messages - consider addressing them');
    }
  }
  ...
  await for (final file in Directory('lib').list(recursive: true)) {
    if (file is File && file.path.endsWith('.dart')) {
      ...
      if (content.contains('TODO') && content.toLowerCase().contains('security')) {
        ...
        for (int i = 0; i < lines.length; i++) {
          ...
          if (line.contains('sharedpreferences') && 
              (line.contains('token') || 
               ...
               line.contains('key'))) {
            issues.add('Potential sensitive data in SharedPreferences: ${file.path}:${i+1}');
            score -= 20;
          }
        }
      }
...
Future<AuditResult> checkLoggingSecurity() async {
  ...
  if (printCount > 0) {
    issues.add('Found $printCount files using print() instead of secure logger');
...
Future<AuditResult> checkAuthenticationSecurity() async {
  ...
  await for (final file in Directory('lib').list(recursive: true)) {
    if (file is File && file.path.endsWith('.dart')) {
      final content = await file.readAsString();
      if (content.contains('sessionTimeout') || content.contains('session_timeout')) {
